...
init_numbers (struct dali_config *c)
{
  ...
  state->date_mode = state->time_mode = UNDEFINED_DISPLAY_MODE;
  state->time_mode = c->time_mode;
  state->date_mode = c->date_mode;
  ...
  if (!(c->countdown.Hi || c->countdown.Lo))
    {
      ...
      UInt32 mm, hh, tt;
      UInt32Sub(&delta, &(c->countdown), &time->tv_sec);
          ...
          byte current_frame_num)
{
  ...
  byte i = 0, x;
  byte *cache = _step_cache[current_frame_num];
...
tick_sequence (struct dali_config *c)
{
  ...
  if (state->current_frame >= 10) 
    {
      ...
      start_sequence (c, &now, !state->force_refresh);
      if (state->force_refresh) {
        state->force_refresh = FALSE;
        state->current_frame = 7;
        for (int i = 0; i < countof(state->target_digits); i++)
          state->orig_digits[i] = 0xff;
      } else {
  ...
  for (i = 0; i < countof (state->current_frames); i++)
    one_step (c,
              ...
              state->target_frames[i],
              state->current_frame > 7 ? 7 : state->current_frame);
}
...
compute_left_offset (struct dali_config *c)
{
  ...
  if (state->target_digits[0] == 0xff &&	/* Fading in to no digit */
    ...
    c->left_offset = state->char_width / 2;
           ...
           state->target_digits[0] != 0xff)
    c->left_offset = state->char_width * (8 - state->current_frame) / 16;
  else if (state->orig_digits[0] != 0xff &&	/* Anim digit to no digit. */
           state->target_digits[0] == 0xff)
    c->left_offset = state->char_width * state->current_frame / 16;
...
draw_clock (struct dali_config *c)
{
  ...
  x = (byte)(c->width - ((nn * state->char_width) + (cc * state->colon_width)))/2;
  y = (byte)((c->height - state->char_height)/2);
  for (i = 0; i < nn+cc; i++) 
    {
...
render_init (struct dali_config *c)
{
  ...
  for(byte jj=0; jj<8; jj++)
    for(byte ii=0; ii<32; ii++)
      _step_cache[jj][ii] = (byte)((jj * (int)ii)/7);
...
render_once (struct dali_config *c)
{
  ...
  struct render_state *state = c->render_state;
  byte refresh = (state->date_mode != c->date_mode) || (state->time_mode != c->time_mode);
  if (refresh) {
    state->current_frame = 0xfe;
    memset(c->bitmap, 0xff, 6144);
    memset(state->target_digits, 0xff, sizeof(state->target_digits));
    state->date_mode = c->date_mode;
    state->time_mode = c->time_mode;
